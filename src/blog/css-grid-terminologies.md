...
CSS Grid came into limelight recently([few years ago](https://www.youtube.com/watch?v=Felq4z_rdPQ)) but its presence has been there since old IE days. CSS Grid solves the problem area of two dimensional layouts. There has been multiple attempts to solve the designing layout using other properties in css - position hacks, floats, table, flex are such very popular examples with comes with their own set of problems.

![i don't need grid](/images/css-grid-terminologies/i-have-flexbox.png){loading="lazy"} {.excalidraw-dark}

Syntactically and visually, we can achieve the 2D layouts using flexbox or tables then why do we need grid in picture? Grid brings a very unique quality with itself – **Semantics**. Tables are semantically for table layouts or tabular structured data, flexbox is semantically there for 1 dimensional flexible layouts (navigation bars, etc). In that same essence, Grids are for 2 dimensional layouts.
...
When grid was not available we used tables and flexbox to create a grid like layouts which was not semantically correct and also hard to pull off in some way. Tables and flexbox usages still hold place today for what they were meant to be. At the end, **CSS Grid is just another tool in our toolbox**. And as the saying goes - _use the right tool, for the right job._ It blends well with other layout systems like it is perfectly valid to create a flex box container inside a css grid element.
...
To answer that, lets understand what is _formatting context_ in general. Formatting contexts define the rules on how the content is going to layout visually and with combination of other properties, how it effects the layout. This is not something new in CSS, it has been around since the beginning. You may not have known the term before, but for sure you have experienced it.

For example, when you use `display: flex;` it sets the element to be of _flex formatting context_ and then by using flex related properties we can change the layout of its child. There are many such formatting contexts, like when you use a `<p></p>` tag, it creates a _inline formatting context_ which alters the behaviour of box model as compared to the default _block formatting context_.
...
Coming back to grid formatting context. How do we create such thing? By adding the declaration `display: grid;` or  `display: inline-grid;` to an element. With this we create a new grid formatting context and the element will be known as **grid container**.

### Block level and Inline level grid container

We can create grid containers in two fashion – block level and inline level. Block level grid follows the block direction (top-to-bottom for _most_ languages) and gets placed on its own line like a `div, section, p`. Inline level grid, follows the inline direction (left-to-right) placed along other inline elements for eg. `span, strong`.
...
> 📌 If inline-grid container has `position: absolute;` or `float` in its properties, then `inline` gets dropped and it becomes regular block grid container.

### Block grid and Block containers

Block grids looks like they behave similar to the regular block containers which are created by elements such as - div, main, section, etc.. But there are few differences between block level grid and block containers:
- Interaction with floated elements
- Margin collapsing

**Interaction with floated elements** — When a parent element contains a floated element, the child  `div` (or any other block element) goes under the floated element. But in case of grid container, **it does not**. Check out the pen below.
...
Grid lines are the basic unit of grid on which other parts of grid are built upon. How do we create grid lines? We don't. It's something that gets created (_logically_) when we define grid template in case of explicit grid and when grid automatically places content in case of implicit grid. We'll uncover what explicit and implicit grid are in other post.

![grid lines](/images/css-grid-terminologies/grid-lines.png){loading="lazy"} {.excalidraw-dark}

We can refer the grid lines as numbers via css and place the grid items. They start as 1 from from the starting edge of flow direction.

## Grid track

![grid tracks](/images/css-grid-terminologies/grid-tracks.png){loading="lazy"} {.excalidraw-dark}

Area between 2 adjacent grid lines are known as a grid track. The direction of grid track can be vertical and horizontal. Vertical direction are the columns and horizontal direction are rows. (this changes based on the writing mode)

## Grid cell

![grid cells](/images/css-grid-terminologies/grid-cells.png){loading="lazy"} {.excalidraw-dark}

Area between of 4 grid lines is called grid cell, it is the smallest unit of area in grid.
...
Grid area is the combination of one or more grid cells together. These are accessible via css grid properties. We can define the grid areas and place our grid items over those areas.
...
Since the control of assigning grid items to grid areas are on us, we can keep some of the grid cells as empty or overlap them with other grid items.
...
Here we had define only 2 rows, those rows/tracks are part of the **explicit grid**. If the number of grid items increases which cannot fit inside the explicit area of grid, then grid will generate new row automatically as part of **implicit grid**. Similarly for columns.

If we place a grid item outside of the defined row, columns. New grid lines and tracks will be created as part of implicit grid.
